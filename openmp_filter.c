...
//   Insertion sort (thread-local window)
...
Image *median_filter_openmp(const Image *src, int kernel_size, int num_threads)
{
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp for schedule(dynamic, 16) collapse(1)
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x++) {
                int cnt = 0;
...
int main(int argc, char *argv[])
{
    if (argc < 5) {
        fprintf(stderr,
            "Usage: %s <input.pgm> <output.pgm> <noise_ratio> <kernel_size> [num_threads]\n"
            "  noise_ratio  : 0.0 – 1.0  (e.g. 0.05 = 5%%)\n"
            "  kernel_size  : odd integer (e.g. 3, 5, 7)\n"
            "  num_threads  : optional, default = max available\n",
            argv[0]);
        return EXIT_FAILURE;
    }
    ...
    int    num_threads   = (argc >= 6) ? atoi(argv[5]) : omp_get_max_threads();
    ...
    printf("Threads      : %d (max available: %d)\n\n",
    ...
    // 4. Thread benchmark across different thread counts 
    ...
    Image *best_filtered = NULL;
    double best_time = 1e18;
    int    best_threads = num_threads;

    int thread_counts[] = {1, 2, 4, 8, 16};
    int nc = sizeof(thread_counts) / sizeof(thread_counts[0]);

    for (int ti = 0; ti < nc; ti++) {
        int tc = thread_counts[ti];
        if (tc > omp_get_max_threads()) continue;

        t0 = now_ms();
        Image *f = median_filter_openmp(noisy, kernel_size, tc);
        double elapsed = now_ms() - t0;

        double psnr_f = compute_psnr(orig, f);
        printf("%-10d %-16.4f %-12.2f %-12.2f\n",
               tc, elapsed, psnr_noisy, psnr_f);

        if (elapsed < best_time) {
            best_time    = elapsed;
            best_threads = tc;
            if (best_filtered) free_image(best_filtered);
            best_filtered = f;
        } else {
            free_image(f);
        }
    ...
    // 5. Save best result 
    if (best_filtered) {
        write_pgm(out_file, best_filtered);
    }

    double psnr_filtered = best_filtered ? compute_psnr(orig, best_filtered) : 0.0;
